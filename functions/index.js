...
exports.lmno_callback_url = functions.https.onRequest(async (req, res) => {
   ...
   let providerUid = req.query.uid.split("/")[2];
    ...
    let transaction = admin.firestore().collection('mpesa').doc('transaction').collection(uid).doc('/' + parsedData.checkoutRequestID + '/');
    let wallet = admin.firestore().collection('providers').doc(providerUid).collection('account').doc('analytics');
